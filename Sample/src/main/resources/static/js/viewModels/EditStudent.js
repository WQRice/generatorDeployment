...
define(['ojs/ojcore', 'knockout', 'ojs/ojcheckboxset'
    ...
    function viewModel() {
        ...
        self.initialize = function () {
            ...
            self.inputStudentId(rootViewModel.frontToEditData().id);



self.inputStudentstudentId(rootViewModel.frontToEditData().studentId);

self.inputStudentfirstName(rootViewModel.frontToEditData().firstName);

self.inputStudentlastName(rootViewModel.frontToEditData().lastName);
            ...
            for (var i in self.oneArray) {
                var url = 'http://localhost:8080/Student/'+self.inputStudentId()+"/"+self.attMap[self.oneArray[i]].attInSelfName;
            ...
            // for one to many relationship
                ...
                for(var j in self.manyArray) {
                    var url = 'http://localhost:8080/Student/'+self.inputStudentId()+"/"+self.attMap[self.manyArray[j]].attInSelfName;
...
self.inputStudentCourseArray = ko.observableArray().extend({ deferred: true });
self.inputCourseDataProvider = new oj.ArrayDataProvider(self.inputStudentCourseArray, {keyAttributes: 'id'});
...
"headerStyle" : 'font-weight:bold'
        ...
        self.updateRow = function () {
            ...
            $.ajax({
                ...
                url: "http://localhost:8080/Student/" + self.inputStudentId(),
                data: JSON.stringify(
                    {
            ...
            for (var j in self.manyArray) {
                ...
                var attAssociationUrl = "http://localhost:8080/Student/" + self.inputStudentId() + "/" + self.attMap[self.manyArray[j]].attInSelfName;
                        ...
                        for (var i = 0; i < oldList[self.manyArray[j]].length; i++) {
                            ...
                            if (overlap!==true) {
                                ...
                                queuechain1 = queuechain1.then(
                                    $.ajax({
                                        url: "http://localhost:8080/Student/" + self.inputStudentId() + "/" + self.attMap[self.manyArray[j]].attInSelfName +"/" + oldList[self.manyArray[j]][i].id,
                                ...
                                queuechain1 = queuechain1.then(
                                    $.ajax({
                                        url: "http://localhost:8080/"+self.manyArray[j]+"/" + oldList[self.manyArray[j]][i].id + "/studentIn" + self.manyArray[j]+"/" + self.inputStudentId(),
                                        type: "DELETE",
                                        success: function (response) {
                                            //alert("delete success");
                        ...
                        //alert("input length many" +JSON.stringify(self[inputAttRefMany]()));
                            ...
                            for (var jj = 0; jj < self[inputAttRefMany]().length; jj++) {
                                ...
                                if (overlap != true) {
                                    ...
                                    var associationUrl = "http://localhost:8080/api/Student/" + self.inputStudentId() + "/" + self.manyArray[j] + "/" + self[inputAttRefMany]()[jj].id;
            ...
            for (var j in self.oneArray) {
                ...
                if (self[inputAttRef]().length > 0) {
                    ...
                    var associationUrl = "http://localhost:8080/api/Student/" + self.inputStudentId() + "/" + self.oneArray[j] + "/" + self[inputAttRef]()[0].id;
                   ...
                   // alert(associationUrl);
                    ...
                    var attAssociationUrl = "http://localhost:8080/Student/" + self.inputStudentId() + "/" + self.attMap[self.oneArray[j]].attInSelfName;
                    ...
                    $.ajax({
                        ...
                        function (output) {
                           // alert(JSON.stringify(output));
                            ...
                            queuechain2 = queuechain2.then(
                                $.ajax({
                                    url: "http://localhost:8080/"+ self.oneArray[j]+"/" + output.id + "/studentIn" + self.oneArray[j]+"/" + self.inputStudentId(),
                                    type: "DELETE",
                                    success: function (response) {
                                      //  alert("delete success one");
                                        console.log(response);
